...
**Current phase:** Open Beta — Netwerk. Visibility, email, membership, uptime, schema-invariant, and commenting work is merged to `develop`; Activity Notifications is in review. The gating step is the prod-migration cutover to `main`.
...
- The [`emitActivity()` dispatcher](https://app.notion.com/p/38d453d029b08159b29cef84711e9a75) now ships in-app notifications (netwerk-6). Remaining follow-up: [authenticated member RSVP (`Rsvp.userId`)](https://app.notion.com/p/38d453d029b081c092c6fb5c85536720), which lights up the currently-inert RSVP-actor guard. Email + preference channels are still deferred to their own tickets.
