...
**Current phase:** Open Beta — Netwerk. Visibility, email, membership, uptime, schema-invariant, and commenting work is merged to `develop`; Activity Notifications is in review and Email Notifications is built on `netwerk-7`. The gating step is the prod-migration cutover to `main`.
...
- **Built, pre-PR:** Email Notifications (`netwerk-7-email-notifs`) — enqueue→~15-min flush that coalesces + read-suppresses, per-(user,context) preferences with per-context master + one-click unsubscribe, settings UI. 358 unit tests green. Its migration is additive (safe in the cutover). **Prod config is a P0 prereq:** [FLUSH_SECRET + unsubscribe secret + migration](https://app.notion.com/p/3a5453d029b0811098f6c5d562620177) — the flush endpoint fails closed without `FLUSH_SECRET` (set in Vercel *and* as a GitHub Actions secret).
